...
Встроенный Telegram-прокси на библиотеке `mtglib` с поддержкой **FakeTLS (маскировка под доверенные TLS-домены)**. Работает прямо в памяти процесса без дочерних процессов и внешних форков:
* **Выделенный режим (Standalone):** управление через карточку «MTProto Proxy» на вкладке «Сервера» (аналогично внешним серверам). Запуск через `rospanel mtproto run` в ультралегком режиме с потреблением памяти **менее 30 МБ RSS** — оптимизировано для работы в контейнерах с жестким лимитом памяти (Pterodactyl 96 МБ, минимальные VPS). В бинарнике поднимается только ядро прокси и фоновая синхронизация с панелью (никаких тяжелых сервисов, SQLite или веб-сервера). Включает встроенный pprof (`--pprof-addr`) для профилирования и диагностики производительности.
* **Смешанный режим (Mixed Node Mode):** включается в настройках ноды (или мастера) на вкладке «Основное» в блоке «MTProto Proxy» под системным прокси. Нода запускает встроенный прокси параллельно с Xray, а в списке Telegram-прокси отображается статус, аптайм, память и активные соединения.
* **Управление и шаринг:** генерация криптографически стойких FakeTLS-секретов (`ee...`), готовые deep-link ссылки `tg://proxy?server=...&port=...&secret=...` и `https://t.me/proxy?...`, генерация QR-кода и готовой команды установки в один клик.
...
### Режим MTProto Proxy (Telegram)

Автономный запуск выделенного Telegram-прокси в минимальном объеме памяти (< 30 МБ RSS):

```text
rospanel mtproto run               запустить MTProto-прокси (флаги: --port, --secret, --domain, --max-conns, --sync-url, --sync-token, --pprof-addr)
rospanel mtproto gen-secret        сгенерировать FakeTLS-секрет (флаг: --domain)
```

Команда запуска со всеми параметрами и токеном синхронизации генерируется в панели при добавлении прокси.
