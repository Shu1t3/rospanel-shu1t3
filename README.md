...
[Что это](#что-это) · [Быстрый старт](#-быстрый-старт) · [Миграция с оригинала](#-миграция-с-оригинальной-версии-appsganinrospanel) · [Возможности](#-возможности) · [CLI](#️-cli) · [Архитектура](#-архитектура) · [Дисклеймер](#️-дисклеймер) · [Разработка](#-разработка) · [Благодарности](#-благодарности) · [Лицензия](#-лицензия)
...
**Способ 3 — импорт из Happ Subscription.** Вкладка «Happ Subscription» в диалоге добавления сервера: вставьте ссылку на подписку (`happ://crypt...`, `https://...` или base64-список). Панель нативно расшифровывает форматы (crypt1–crypt5), парсит ноды (VLESS, VMess, Trojan, Shadowsocks, Hysteria2) и подключает их как исходящие соединения Xray (outbounds). Ноды автоматически обновляются в фоне каждые 59 минут, отображаются в разделе «Сервера» с компактными карточками и возможностью быстрого включения/отключения.
...
попадают в Clash/sing-box подписки.

**AmneziaWG 3.1 (вкладка «Доп. подключения»)** — нативная поддержка протокола AWG 3.1 в userspace Go (`amneziawg-go/v3`, без необходимости сборки ядерных модулей kmod). Полная настройка продвинутой обфускации рукопожатий и пакетов: мусорные пакеты Jc/Jmin/Jmax, паддинги S1–S4, диапазоны H1–H4, защита заголовков `HeaderProtectionKey`, `ContentPaddingAddition`, флаги `RandomTrailers` и `DisableCookies`, а также готовые пресеты сигнатур инициализации I1–I5 (QUIC Initial, TLS ClientHello, DNS Query, HTTP/3, DTLS, WG Noise). Состояние туннеля непрерывно отслеживается на мастере и удалённых нодах, отображается в диагностике системы и экспортируется в метрики Prometheus (`rospanel_node_awg_running`).

Панель спрятана за **секретным путём**, любой другой путь отдаёт сайт-заглушку (11 шаблонов). **Детект
...
**лимит устройств** (что именно он считает — см. *Привязка устройств* ниже) и **лимит скорости**. Учёт трафика через **прямой in-process gRPC-клиент** к Xray StatsService (без запуска внешних процессов CLI и с мгновенным Protobuf-декодированием), серверная пагинация API (`ListUsersPaged`) без задержек на тысячах пользователей, онлайн-статус,
список подключений; истёкших можно авто-удалять. Поиск и фильтры работают мгновенно на сотнях и тысячах
...
клиенты (авто-роутинг-заголовки Happ / INCY / Mihomo), со своими названиями узлов.
**Унифицированный движок подписок** (`sub.Request`) с высокопроизводительной генерацией
профилей (Sing-Box JSON, Clash Meta YAML, универсальные ссылки, HTML-страница): генерация Sing-box
ускорена в 2.7 раза со статическими структурами и минимальными аллокациями. Внешние серверы
(Happ Subscription) отдаются клиентам изолированно, даже если все локальные ноды переполнены
или скрыты. Ссылку можно **сбросить** (ротация токена) без смены UUID и паролей. **Объявление** внутри клиента
...
с выбором региона. Лейн WARP опирается на **несколько эндпоинтов Cloudflare одновременно** — разные подсети адресов и разные порты UDP с проверкой здоровья (Observatory), поэтому падение одного диапазона или порта не ломает выход; в качестве пиров пишутся только явные IP (без доменных имен), исключая зависимость от DNS и скрытые утечки. **Полосы прокси** — независимые выходы, у каждого свои socks5/http-апстримы
и правила по зонам с балансировкой по живым (Observatory). При временном сбое внешнего URL со списком прокси панель сохраняет последний успешный список (Last-Good Cache) вместо мгновенного опустошения полосы. Переключатель **«Никогда не сбрасывать в direct при падении выхода» (StrictEgress)** гарантирует, что если все узлы полосы недоступны или аккаунт WARP не привязан, трафик сбрасывается (`block`), а не уходит напрямую с белого IP сервера. **Снимки конфигурации** (вкладка
...
#### 🔄 Внешние подписки и Happ Subscription (прокси-цепочки и переиспользование)

Панель позволяет импортировать внешние прокси-подписки и бесшовно использовать их серверы в собственной инфраструктуре:

* **Поддерживаемые форматы и нативная расшифровка:** импорт обычных URI-списков (VLESS, VMess, Trojan, Shadowsocks, Hysteria2), Base64-подписок и зашифрованных ссылок `happ://crypt`, `happ://crypt2`, `happ://crypt3`, `happ://crypt4` и `happ://crypt5` (RSA-1024 / RSA-4096 / ChaCha20-Poly1305 keytable). Реализовано нативно на основе исследований и спецификаций [happ-decryptor](https://leeeet.dev/happ-decryptor/) (исходный код: [LeeeeT/happ-decryptor](https://github.com/LeeeeT/happ-decryptor)).
* **Автоматическое фоновое обновление:** подписки обновляются каждые 59 минут с автоподтягиванием новых ключей, доменов и портов. Есть ручное обновление одной или всех подписок.
* **Исходящие Xray Outbounds и прокси-цепочки:** каждый включенный узел регистрируется в ядре как `happ-<id>` и доступен для построения цепочек прокси (proxy chaining) и полос маршрутизации (Egress Lanes).
* **Включение в подписки клиентов:** активные серверы из Happ-подписок автоматически транслируются в клиентские подписки во всех форматах: универсальные ссылки / Base64, Sing-Box (JSON outbounds + селекторы) и Clash Meta / Mihomo (YAML proxies).
* **Интеграция с группами доступа (Access Groups):** оператор может гибко раздавать конкретные Happ-серверы определенным группам пользователей через чекбоксы с бейджем `Happ`.
* **Информационные заглушки (`Subscription expired`):** автоматическое распознавание сервисных заглушек провайдеров (`0.0.0.0:1`, `expired`, `истекла`) с предупреждающим бейджем `⚠️ Инфо / Заглушка` и поясняющей подсказкой (создаются отключенными для защиты Xray от сбоев).
* **Компактный интерфейс:** карточки подписок сворачиваются в 1 строку со счетчиками (`X из Y активны`); модальное окно позволяет выполнять поиск по странам/названиям и пакетно включать/отключать серверы («Включить все» / «Отключить все»).
...
Уведомление — не единственная реакция. На этой же вкладке настраиваются **автоматические меры** — лестница из трёх ступеней с дневным порогом совпадений (0 выключает ступень): **предупредить** пользователя через его Telegram-бота (раз в сутки), **ограничить скорость** до заданного потолка, **отключить доступ**. Ограничение скорости и отключение действуют заданное время (от часа до 30 дней), после чего панель возвращает прежнее состояние и сообщает пользователю; каждое действие попадает в журнал. Ручное включение пользователя или изменение скорости отменяет меру. Трафик нод тоже учитывается: совпадения агрегируются на мастере, а меры доезжают до нод через штатную синхронизацию.
...
Совпадения хранятся **30 дней** — этого хватает на разбор жалобы.

**Откуда разрешено подключаться** (*Настройки → Основные*). Правило по странам (только эти или отовсюду кроме этих) плюс список **сетей (ASN)**, которым подключение запрещено — классический признак перепродажи учёток хостинг-провайдерам. Проверяется для каждого соединения на мастере и на каждой ноде, закрывая любые протоколы. **Сбрасывается адрес, а не аккаунт**: IP нарушителя попадает в nftables-набор на всех серверах с настраиваемым временем жизни, а аккаунт продолжает работать отовсюду, где разрешено. Принудительный сброс опционален — до включения нарушения просто фиксируются в журнале и на странице статистики. Адрес, который geo-таблица не может определить, никогда не блокируется — отсечение реальных пользователей недопустимо.

#### 🧰 Эксплуатация и безопасность

**Алерты входа администраторов**. При входе с нового адреса Telegram-бот отправляет уведомление в чат админов с указанием IP, страны, провайдера и кнопкой «Не я», мгновенно отзывающей все сессии этого администратора.

**Диагностика** одной кнопкой: процесс Xray, туннель AmneziaWG (на мастере и нодах), применение конфига, срок TLS, место на диске,
...
восстановление** и сброс — из панели и CLI. При восстановлении запрашивается пароль и код 2FA (если включена), а если у администраторов внутри архива бэкапа была настроена двухфакторка — запрашивается и её код (включая мастер первоначальной настройки).
...
при обновлении панели ноды и мастер сверяют версию Xray и обновляют её автоматически;
упавшее супервизор поднимает сам. **Сторож** закрывает более сложный случай, который не видит
обработчик краха, — процесс жив, но перестал обслуживать: он опрашивает API Xray по прямому gRPC-каналу
(`PingAPI`) и, если тот не отвечает несколько проверок подряд, перезапускает его (с задержкой против шторма
...
инбаундах банится iptables, а число соединений с одного IP ограничено nftables (с пакетным
применением правил `BlockIPs`/`UnblockIPs` единой транзакцией).
...
Архитектура РосПанели построена по принципу **разделения плоскости управления (Control Plane) и плоскости передачи данных (Data Plane)**:

* **Control Plane (Панель управления):** единый бинарник на Go, оркестрирующий состояние системы, веб-интерфейс, генерацию конфигураций и подписок, фоновые задачи и синхронизацию нод. Панель может работать совместно с локальным ядром Xray либо в автономном режиме Control Plane. Для изолированного локального управления без сетевого оверхеда поддерживается слушатель Management API через Unix Domain Socket (`ROSPANEL_UNIX_SOCKET`).
* **Внутреннее взаимодействие с Xray:** сбор метрик трафика (`StatsService`) и сторожевой опрос живости (`PingAPI`) выполняются через прямой in-process **gRPC-клиент** (`127.0.0.1:10085`) без спавна внешних процессов CLI (`xray api statsquery`). Это исключает накладные расходы `fork()`/`execve()` и ускоряет парсинг Protobuf-статистики в 10 раз.
* **База данных и кэширование:** единственный источник правды — **SQLite** в режиме WAL (`PRAGMA journal_mode=WAL; PRAGMA synchronous=NORMAL;`). Поддерживается пул параллельных читателей с мьютексом `writeMu` (гарантированная защита от ошибок `SQLITE_BUSY`) и атомарный in-memory кэш настроек (`atomic.Pointer[model.Settings]`) с задержкой чтения < 400 нс.
* **Изоляция слоёв (DTO):** строгая граница между внутренними доменными сущностями и внешними сетевыми контрактами. Спецификация OpenAPI и REST API оперируют типизированными DTO-структурами (`internal/server/dto.go`).
* **Синхронизация узлов (Node Agent):** агент ноды взаимодействует с панелью по исходящему HTTPS long-poll, передавая срез состояний компонентов `ComponentStatus` (Xray, AmneziaWG). Супервизор мастера осуществляет детерминированную агрегацию статусов всей сети.

**Стек:** Go 1.27 · Xray-core · AmneziaWG 3.1 (userspace Go) · SQLite (modernc, без CGO) · React + Vite + Tailwind.
...
`ROSPANEL_UNIX_SOCKET` (путь к Unix Domain Socket для Management API),
...
### Стандарты коммитов и ведение CHANGELOG

PR и issue приветствуются. Коммиты следуют спецификации [Conventional Commits](https://www.conventionalcommits.org/): на их основе [release-please](https://github.com/googleapis/release-please) автоматически собирает релиз, формирует [CHANGELOG.md](CHANGELOG.md) и публикует бинарники и Docker-образы в GHCR.

**Для сохранения подробной технической истории изменений:**
1. **Подробное тело коммита (Commit Body):** заголовок коммита (`feat:`, `fix:`, `perf:`, `refactor:`) обязательно дополняется структурированным описанием в теле коммита: какие модули изменены, в чём причина решения и каковы измеримые результаты (бенчмарки, задержки, экономия памяти).
2. **Обогащение Release PR:** release-please формирует черновик PR релиза из первых строк коммитов. Перед слиянием релизного PR необходимо обогащать секции в [CHANGELOG.md](CHANGELOG.md) детальными пунктами из тел коммитов.
3. Полный регламент зафиксирован в [.agents/rules/changelog-and-commits.md](.agents/rules/changelog-and-commits.md).

---

## 🙏 Благодарности

* [Xray-core](https://github.com/XTLS/Xray-core) — высокопроизводительное модульное ядро маршрутизации и проксирования.
* [happ-decryptor](https://leeeet.dev/happ-decryptor/) / [LeeeeT/happ-decryptor](https://github.com/LeeeeT/happ-decryptor) — реверс-инжиниринг и спецификация криптографических схем расшифровки подписок Happ Proxy Subscriptions.
* [AppsGanin/rospanel](https://github.com/AppsGanin/rospanel) — исходная архитектурная основа проекта.
