...
**Способ 3 — импорт по ссылке (аренда ноды).** Вкладка «Импорт по ссылке» в диалоге добавления ноды: вставьте защищённую ссылку `rpnshare://`, которой с вами поделился владелец другой панели RosPanel. Арендованная нода сразу подключится в вашу панель с выделенной квотой и ограничением скорости, позволяя создавать свои подключения на свободных портах.

**Способ 4 — импорт из Happ Subscription.** Вкладка «Happ Subscription» в диалоге добавления сервера: вставьте ссылку на подписку (`happ://crypt...`, `https://...` или base64-список). Панель нативно расшифровывает форматы (crypt1–crypt5), парсит ноды (VLESS, VMess, Trojan, Shadowsocks, Hysteria2) и подключает их как исходящие соединения Xray (outbounds). Ноды автоматически обновляются в фоне каждые 59 минут, отображаются в разделе «Сервера» с компактными карточками и возможностью быстрого включения/отключения.
...
#### 🤝 Аренда и совместное использование нод (деление ресурсов)

Владелец ноды может **поделиться её мощностями** с администраторами других панелей RosPanel через зашифрованную ссылку формата `rpnshare://` без передачи SSH-доступа и секретов всей панели:

* **Главенство владельца (Owner Supremacy):** первичный владелец сохраняет полный контроль над машиной. Он задаёт процент доступной квоты трафика и потолок скорости (Kbps), видит список всех активных арендаторов (их трафик, скорость, последний онлайн) и может отозвать доступ в любой момент.
* **Равномерное распределение ресурсов:** выделенная квота и лимит скорости автоматически и динамически делятся поровну между всеми подключенными арендаторами.
* **Зарезервированные порты:** в редакторе подключений наглядно отображаются все занятые системные порты владельца и арендаторов — новые подключения создаются только на свободных портах.
* **Безопасная маршрутизация:** системный security floor панели (изоляция приватных сетей, блокировка служебных IP) и глобальные правила блокировки владельца компилируются с наивысшим приоритетом и действуют поверх конфигурации арендаторов.
* **Локальное удаление:** при удалении арендованной ноды арендатор отключает её только из своей панели; все созданные им подключения на удалённом сервере автоматически и каскадно очищаются, а нода владельца продолжает работать без изменений.
* **Визуальное разделение:** арендованные ноды элегантно выделяются в интерфейсе в отдельный блок с акцентным фреймингом и бейджем, не перегружая основной список серверов.
...
**Стек:** Go 1.27 · Xray-core · SQLite (modernc, без CGO) · React + Vite + Tailwind.
