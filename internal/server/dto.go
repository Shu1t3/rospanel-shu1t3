...
type nodeRentalSettingsDTO struct {
...
func toNodeRentalSettingsDTO(s *model.NodeRentalSettings) nodeRentalSettingsDTO {
	if s == nil {
		return nodeRentalSettingsDTO{}
	}
	return nodeRentalSettingsDTO{
		ShareEnabled:      s.ShareEnabled,
		ShareQuotaPercent: s.ShareQuotaPercent,
		ShareSpeedLimit:   s.ShareSpeedLimit,
		ShareToken:        s.ShareToken,
		MaxTenants:        s.MaxTenants,
	}
}

func fromNodeRentalSettingsDTO(d nodeRentalSettingsDTO) model.NodeRentalSettings {
	return model.NodeRentalSettings{
		ShareEnabled:      d.ShareEnabled,
		ShareQuotaPercent: d.ShareQuotaPercent,
		ShareSpeedLimit:   d.ShareSpeedLimit,
		ShareToken:        d.ShareToken,
		MaxTenants:        d.MaxTenants,
	}
}

type nodeTenantDTO struct {
	ID          int64  `json:"id"`
...
func toNodeTenantDTO(t *model.NodeTenant) nodeTenantDTO {
	if t == nil {
		return nodeTenantDTO{}
	}
	return nodeTenantDTO{
		ID:          t.ID,
		NodeID:      t.NodeID,
		TenantID:    t.TenantID,
		Name:        t.Name,
		TrafficUp:   t.TrafficUp,
		TrafficDown: t.TrafficDown,
...
func toNodeTenantDTOs(tenants []model.NodeTenant) []nodeTenantDTO {
	if tenants == nil {
		return []nodeTenantDTO{}
	}
	out := make([]nodeTenantDTO, len(tenants))
	for i := range tenants {
		out[i] = toNodeTenantDTO(&tenants[i])
	}
	return out
}
...
func toPortInfoDTO(p *model.PortInfo) portInfoDTO {
	if p == nil {
		return portInfoDTO{}
	}
	return portInfoDTO{
...
func toPortInfoDTOs(ports []model.PortInfo) []portInfoDTO {
	if ports == nil {
		return []portInfoDTO{}
	}
	out := make([]portInfoDTO, len(ports))
	for i := range ports {
		out[i] = toPortInfoDTO(&ports[i])
	}
	return out
}
