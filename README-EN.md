...
#### 🛡️ MTProto Proxy (Telegram)

Embedded Telegram proxy built on `mtglib` with **FakeTLS (domain fronting / TLS masquerading)**. Operates directly in-memory without child processes or external forks:
* **Standalone Mode:** managed via the "MTProto Proxy" card on the "Servers" tab (analogous to external servers). Launched with `rospanel mtproto run` in an ultra-low memory mode consuming **under 30 MB RSS** — optimized for minimal VPS and resource-constrained environments like Pterodactyl containers (96 MB limit). Only the core proxy engine and panel sync run (no heavy services, SQLite, or web routes). Includes embedded pprof (`--pprof-addr`) for profiling and diagnostics.
* **Mixed Node Mode:** enabled inside Node (or Master) settings on the "General" tab in the "MTProto Proxy" section below the system proxy. The node agent supervises MTProto alongside Xray, and the node appears in the unified Telegram proxies list with status, uptime, memory, and active connections.
* **Management & Sharing:** one-click cryptographic FakeTLS secret generation (`ee...`), ready deep links `tg://proxy?server=...&port=...&secret=...` and `https://t.me/proxy?...`, QR codes, and auto-generated install commands.
...
### MTProto Proxy (Telegram) Mode

Standalone low-footprint execution of the Telegram proxy (< 30 MB RSS):

```text
rospanel mtproto run               launch MTProto proxy (flags: --port, --secret, --domain, --max-conns, --sync-url, --sync-token, --pprof-addr)
rospanel mtproto gen-secret        generate FakeTLS secret (flag: --domain)
```

The full launch command with sync tokens and parameters is generated by the panel when adding a proxy.
